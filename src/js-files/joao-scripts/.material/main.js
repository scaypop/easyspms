...
export class Accessibility {
  ...
  initFontSize() {
    ...
    if (!this.htmlInitFS) {
      let htmlInitFS = common.getFormattedDim(
        getComputedStyle(this.html).fontSize
      );
      let bodyInitFS = common.getFormattedDim(
        getComputedStyle(this.body).fontSize
      );
    ...
    let css =
      `
        ...
        ._access-icon {
            ...
            transform: ${
              !this.options.icon.useEmojis ? "scale(1)" : "skewX(18deg)"
            };
        }
        ._access-icon:hover {
            ` +
      ...
      `
        ...
        ._access-menu ul li:before {
            ...
            ${
              !this.options.icon.useEmojis
                ? "font-family: " + this.options.icon.fontFamily + ";"
                : ""
            }
  ...
  injectIcon() {
    ...
    let iStyle = `width: ${
      ...
      this.options.icon.dimensions.width.units
            ...
            ;line-height: ${
              lineHeight + this.options.icon.dimensions.width.units
            }
            ;text-indent: ${
              textIndent + this.options.icon.dimensions.width.units
            }
            ;background-color: ${
              !this.options.icon.useEmojis
    ...
    for (let i in this.options.icon.position) {
      iStyle +=
        ...
        i +
        ":" +
        this.options.icon.position[i].size +
        this.options.icon.position[i].units;
    }
    iStyle += `;z-index: ${this.options.icon.zIndex}`;
    let className =
      `_access-icon ${this.options.icon.fontClass} _access` +
  ...
  alterTextSpace(isIncrease) {
    ...
    if (this.options.textPixelMode) {
      ...
      // wordSpacing
      let fSpacing = common.getFormattedDim(
        getComputedStyle(this.body).wordSpacing
      );
      if (typeof this.initialValues.body.wordSpacing === "undefined")
        this.initialValues.body.wordSpacing = "";
      if (fSpacing && fSpacing.sufix && !isNaN(fSpacing.size * 1)) {
        this.body.style.wordSpacing =
          fSpacing.size * 1 + factor + fSpacing.sufix;
      }
      // letterSpacing
      let fSpacing2 = common.getFormattedDim(
        getComputedStyle(this.body).letterSpacing
      );
    ...
    this.menuInterface = {
      ...
      underlineLinks: (destroy) => {
        ...
        if (destroy) {
          ...
          document
            .querySelector(
              '._access-menu [data-access-action="underlineLinks"]'
            )
            .classList.remove("active");
          return remove();
        }
