...
let common = {
  // creates html element from json
  jsonToHtml: (obj, resource) => {
    let elm = document.createElement(obj.type);
    for (let i in obj.attrs) {
      if (i.indexOf("#") === 0 && resource)
        elm.setAttribute(obj.attrs[i], resource[i.substring(1)]);
    ...
    for (let i in obj.children) {
      ...
      if (obj.children[i].type == "#text") {
        if (obj.children[i].text.indexOf("#") == 0)
          newElem = document.createTextNode(
            resource[obj.children[i].text.substring(1)]
          );
        else newElem = document.createTextNode(obj.children[i].text);
      } else if (obj.children[i].type == "#img") {
        if (obj.children[i].text.indexOf("#") == 0)
          newElem = document.createElement(
            resource[obj.children[i].text.substring(1)]
          );
        else newElem = document.createTextNode(obj.children[i].text);
      } else newElem = common.jsonToHtml(obj.children[i], resource);
      if (
        (newElem &&
          newElem.tagName &&
